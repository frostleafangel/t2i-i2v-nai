...
for (const row of rowsToFix) {
    // Actually, sometimes the queue wait time could add up, let's just subtract 28,800,000 unconditionally from these absurdly high ones.
    let newDuration = row.duration_ms - 28800000;
    
    // If somehow the result is negative (maybe the offset was slightly different?), cap it to a sensible default or the actual remainder
    if (newDuration < 0) {
