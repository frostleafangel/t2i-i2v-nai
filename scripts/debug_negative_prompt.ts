...
const TEST_NEGATIVE = {
    "prompt": {
        ...
        "258": {
            ...
            "inputs": {
                "images": ["9", 5],
                "filename_prefix": "Duo_Negative_Test"
            }
        }
    }
};
...
async function main() {
    try {
        console.log("Testing with user negative prompt in Node 39...");

        const response = await fetch(`${COMFY_URL}/prompt`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(TEST_NEGATIVE)
        });
        ...
        if (history[data.prompt_id]) {
            ...
            if (status.status_str === 'error') {
                console.log("FAILED - User negative prompt causes the issue!");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - Negative prompt is not the issue!");
            }
        }
    } catch (error) {
