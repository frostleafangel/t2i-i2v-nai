...
const TEST_WITH_NEWLINE = {
    "prompt": {
        ...
        "258": {
            ...
            "inputs": {
                "images": ["9", 5],
                "filename_prefix": "Duo_NewlineTest"
            }
        }
    }
};
...
async function main() {
    try {
        console.log("Testing with merged negative + trailing newline...");
        console.log("Node 40.文本 (last 20 chars):", JSON.stringify(TEST_WITH_NEWLINE.prompt["40"].inputs["文本"].slice(-20)));

        const response = await fetch(`${COMFY_URL}/prompt`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(TEST_WITH_NEWLINE)
        });
        ...
        if (history[data.prompt_id]) {
            ...
            if (status.status_str === 'error') {
                console.log("FAILED - Newline doesn't help!");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - Newline fix works! Can enable user negative prompt.");
            }
        }
    } catch (error) {
