...
const FRONTEND_EXACT = {
    "prompt": {
        ...
        "40": {
            "class_type": "ZML_TextInput",
            "inputs": {
                "文本": "lowres, worst quality, bad quality, text, watermark, bad anatomy, bad hands, low quality, worst quality, normal quality, text, signature, jpeg artifacts, old, early, copyright name, watermark, artist name, logo, artist logo, patreon logo, blurry, cloned face, cropped, deformed, disfigured, error, out of frame, owres, poorly drawn face, username, furry girl, mole,"
            }
        },
        "61": {
        ...
        "65": {
            "class_type": "ZML_TextInput",
            "inputs": {
                "文本": "1girl, 1girl, white hair, blue eyes"
            }
        },
        "66": {
            "class_type": "ZML_TextInput",
            "inputs": {
                "文本": "1girl, 1girl, black hair, red eyes"
            }
        },
        "67": {
            "class_type": "ZML_PromptUINode",
        ...
        "258": {
            ...
            "inputs": {
                "images": ["9", 5],
                "filename_prefix": "Duo_FrontendExact"
            }
        },
        "269": {
...
async function main() {
    try {
        console.log("Testing EXACT frontend payload...");
        console.log("Node 39.文本1:", JSON.stringify(FRONTEND_EXACT.prompt["39"].inputs["文本1"]));
        console.log("Node 40.文本 length:", FRONTEND_EXACT.prompt["40"].inputs["文本"].length);

        const response = await fetch(`${COMFY_URL}/prompt`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(FRONTEND_EXACT)
        });
        ...
        if (history[data.prompt_id]) {
            ...
            if (status.status_str === 'error') {
                console.log("FAILED - Issue is in frontend payload!");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - Frontend payload works!");
            }
        }
    } catch (error) {
