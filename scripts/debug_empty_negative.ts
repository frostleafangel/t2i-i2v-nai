...
const TEST_EMPTY_NEGATIVE = {
    "prompt": {
        ...
        // Node 39 使用空字符串作为 文本1（不使用用户负面提示词）
        "39": {
            "class_type": "ZML_MultiTextInput3",
            "inputs": {
                "文本1": "", // 空字符串
        ...
        "258": {
            ...
            "inputs": {
                "images": ["9", 5],
                "filename_prefix": "Duo_EmptyNeg_Test"
            }
        }
    }
};
...
async function main() {
    try {
        console.log("Testing with EMPTY user negative prompt...");
        console.log("Node 39.inputs.文本1:", JSON.stringify(TEST_EMPTY_NEGATIVE.prompt["39"].inputs["文本1"]));

        const response = await fetch(`${COMFY_URL}/prompt`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(TEST_EMPTY_NEGATIVE)
        });
        ...
        if (history[data.prompt_id]) {
            ...
            if (status.status_str === 'error') {
                console.log("FAILED - Even empty string doesn't work!");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - Empty negative works! Problem is non-empty user negative prompt.");
            }
        }
    } catch (error) {
