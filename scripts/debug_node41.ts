...
const TEST_NODE41 = {
    "prompt": {
        ...
        // 新增 Node 41 用于用户负面提示词
        "41": {
            "class_type": "ZML_TextInput",
            "inputs": {
                "文本": "lowres, worst quality, bad quality, text, watermark, bad anatomy, bad hands"
            }
        },
        // Node 39 现在通过链接获取用户负面提示词
        "39": {
            "class_type": "ZML_MultiTextInput3",
            "inputs": {
                "文本1": ["41", 0], // 链接到 Node 41
                "文本2": ["40", 0],
                "格式化标点符号": true,
                "分隔符": ",",
                "文本3": ""
        ...
        "258": {
            ...
            "inputs": {
                "images": ["9", 5],
                "filename_prefix": "Duo_Node41_Test"
            }
        }
    }
};
...
async function main() {
    try {
        console.log("Testing with Node 41 for user negative prompt...");
        console.log("Node 39.inputs.文本1:", TEST_NODE41.prompt["39"].inputs["文本1"]);
        console.log("Node 41.inputs.文本:", TEST_NODE41.prompt["41"].inputs["文本"]);

        const response = await fetch(`${COMFY_URL}/prompt`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(TEST_NODE41)
        });
        ...
        if (history[data.prompt_id]) {
            ...
            if (status.status_str === 'error') {
                console.log("FAILED - Node 41 approach doesn't work!");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - Node 41 approach works!");
            }
        }
    } catch (error) {
