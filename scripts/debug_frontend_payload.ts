    ...
    "prompt": {
        ...
        "9": {
            ...
            "inputs": {
                ...
                "latent_image": ["269", 2],
                "optional_vae": ["6", 2],
                "seed": Math.floor(Math.random() * 1000000000000000), // 随机新 seed
                "steps": 30,
                "cfg": 4.5,
        ...
        "39": {
            "class_type": "ZML_MultiTextInput3",
            "inputs": {
                "文本1": "lowres, worst quality, bad quality, text, watermark, bad anatomy, bad hands",
                "文本2": ["40", 0],
                "格式化标点符号": true,
                "分隔符": ",",
                "文本3": ""
        ...
        "61": {
            ...
            "inputs": {
                "model": ["6", 0],
                "clip": ["6", 1],
                "lora_loader_data": "{\"entries\":[{\"id\":\"lora1\",\"item_type\":\"lora\",\"display_name\":\"\",\"custom_text\":\"\",\"lora_name\":\"None\",\"weight\":1,\"enabled\":true,\"parent_id\":null}]}",
                "lora_names_hidden": ""
            }
        },
        "62": {
        ...
        "258": {
            ...
            "inputs": {
                "images": ["9", 5],
                "filename_prefix": "Duo_Debug_Frontend"
            }
        },
        "269": {
...
async function main() {
    try {
        console.log("Sending EXACT frontend payload to ComfyUI...");
        console.log("Resolution preset:", FRONTEND_PAYLOAD.prompt["269"].inputs["预设"]);

        const response = await fetch(`${COMFY_URL}/prompt`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(FRONTEND_PAYLOAD)
        });
        ...
        console.log("Request sent. Waiting 30 seconds then checking status...");
        ...
        if (history[data.prompt_id]) {
            ...
            console.log("Execution status:", status.status_str);
            if (status.status_str === 'error') {
                console.log("FAILED - Same error as frontend!");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - This payload works!");
            }
        } else {
            console.log("No history found, job might still be running.");
        }
    } catch (error) {
