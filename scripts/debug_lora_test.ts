...
// 测试 LoRA 格式差异 - 使用前端的 LoRA 格式但用成功的分辨率
...
async function main() {
    try {
        console.log("Testing with frontend LoRA format + working resolution (832x1216)...");
        ...
        if (history[data.prompt_id]) {
            ...
            if (status.status_str === 'error') {
                console.log("FAILED - LoRA format is NOT the issue (problem persists with working resolution)");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - LoRA format is fine, issue is with SDXL_1024x1024 resolution!");
            }
        }
    } catch (error) {
