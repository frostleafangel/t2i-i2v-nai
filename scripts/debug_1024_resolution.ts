...
// 测试空 LoRA 数组 + SDXL_1024x1024 分辨率
...
async function main() {
    try {
        console.log("Testing empty LoRA + SDXL_1024x1024...");

        const response = await fetch(`${COMFY_URL}/prompt`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(TEST_1024)
        });
        ...
        if (history[data.prompt_id]) {
            ...
            if (status.status_str === 'error') {
                console.log("FAILED - 1024x1024 resolution is the problem!");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - 1024x1024 works with empty LoRA!");
            }
        }
    } catch (error) {
