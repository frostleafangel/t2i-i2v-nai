...
const TEST_EMPTY_LORA = {
    "prompt": {
        ...
        "258": {
            ...
            "inputs": {
                "images": ["9", 5],
                "filename_prefix": "Duo_EmptyLoRA_Test"
            }
        }
    }
};
...
async function main() {
    try {
        console.log("Testing with EMPTY LoRA entries array...");
        console.log("LoRA data:", TEST_EMPTY_LORA.prompt["61"].inputs.lora_loader_data);

        const response = await fetch(`${COMFY_URL}/prompt`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(TEST_EMPTY_LORA)
        });
        ...
        if (history[data.prompt_id]) {
            ...
            if (status.status_str === 'error') {
                console.log("FAILED - Empty LoRA array also fails!");
            } else if (status.status_str === 'success') {
                console.log("SUCCESS - Empty LoRA array works!");
            }
        }
    } catch (error) {
